...
module ice_model_mod
  ...
  public :: update_ice_model_slow_up, update_ice_model_slow_dn, share_ice_domains
  ...
  type :: atmos_ice_boundary_type
     ...
     real, dimension(:,:,:), pointer :: sw_down_vis_dir =>NULL()
     real, dimension(:,:,:), pointer :: sw_down_vis_dif =>NULL()
     real, dimension(:,:,:), pointer :: sw_down_nir_dir =>NULL()
     real, dimension(:,:,:), pointer :: sw_down_nir_dif =>NULL()
...
contains
  ...
  subroutine update_ice_model_slow_up ( Ocean_boundary, Ice )
    ...
    call ice_bottom_to_ice_top (Ice, Ocean_boundary%t, Ocean_boundary%u, Ocean_boundary%v, &
                                Ocean_boundary%frazil, Ocean_boundary, Ocean_boundary%s, &
                                Ocean_boundary%sea_level )
    call mpp_clock_end(iceClock1)
    call mpp_clock_end(iceClock)

  end subroutine update_ice_model_slow_up

  ! This routine does similar tasks to update_ice_model_slow_up for the
  ! post-Verona coupler code.
...
!--------------------------------------------------------------------
  ...
  !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
  ! unpack_land_ice_boundary - store runoff and other fluxes in the ice_data_type. !
  !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
...
subroutine share_ice_domains(Ice)
  type(ice_data_type), intent(inout) :: Ice
  ! This is a null interface to parallel the sharing of domain data
  ! between fast and slow ice PEs in SIS2, but as SIS1 makes no
  ! distinction between the two, nothing needs to be done.
