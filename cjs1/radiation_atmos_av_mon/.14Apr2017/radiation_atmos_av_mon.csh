...
# Model data source:
#
#   pp/${in_data_file_prefix}/av/monthly_Xyr
#
#=======================================================================
#
# Output:
#
#   $out_dir/${in_data_file_prefix}_${yr1}_${yr2}
#
#=======================================================================
#
...
# define output directory using in_data_file prefix

set in_data_file_prefix = `echo $in_data_file | awk -F '.' '{ print $1 }'`

if ( $in_data_file_prefix != 'atmos' && $in_data_file_prefix != 'atmos_cmip' ) then
  echo
  echo 'ERROR: in_data_file_prefix = '$in_data_file_prefix' not valid...'
  echo
  exit 1
endif
...
if ( "$yr1" != "" && "$yr2" != "" ) then
   set out_dir = ${out_dir}/${in_data_file_prefix}_${yr1}_${yr2}
else
...
if ( "$in_data_file_prefix" == "atmos_cmip" ) then
  echo 'adding radiation variables "'$varlist'" into'
  echo $in_data_file
  echo 'using the "atmos_cmip" model radiation variables'
  foreach file ( $in_data_file )
    echo '...'$file
    foreach var ( {$varlist} )
      ...
      switch ( $var )
        case olr:
          set var_result = `ncdump -h -v rlut $file | grep rlut` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rlut $file tmp.nc
            ncrename -v rlut,olr tmp.nc
            ncks -A -v olr tmp.nc $file
          endif
          breaksw
        case olr_clr:
          set var_result = `ncdump -h -v rlutcs $file | grep rlutcs` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rlutcs $file tmp.nc
            ncrename -v rlutcs,olr_clr tmp.nc
            ncks -A -v olr_clr tmp.nc $file
          endif
          breaksw
        case swdn_toa:
          set var_result = `ncdump -h -v rsdt $file | grep rsdt` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rsdt $file tmp.nc
            ncrename -v rsdt,swdn_toa tmp.nc
            ncks -A -v swdn_toa tmp.nc $file
          endif
          breaksw
        case swup_toa:
          set var_result = `ncdump -h -v rsut $file | grep rsut` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rsut $file tmp.nc
            ncrename -v rsut,swup_toa tmp.nc
            ncks -A -v swup_toa tmp.nc $file
          endif
          breaksw
        case swup_toa_clr:
          set var_result = `ncdump -h -v rsutcs $file | grep rsutcs` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rsutcs $file tmp.nc
            ncrename -v rsutcs,swup_toa_clr tmp.nc
            ncks -A -v swup_toa_clr tmp.nc $file
          endif
          breaksw
        case lwdn_sfc:
          set var_result = `ncdump -h -v rlds $file | grep rlds` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rlds $file tmp.nc
            ncrename -v rlds,lwdn_sfc tmp.nc
            ncks -A -v lwdn_sfc tmp.nc $file
          endif
          breaksw
        case lwdn_sfc_clr:
          set var_result = `ncdump -h -v rldscs $file | grep rldscs` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rldscs $file tmp.nc
            ncrename -v rldscs,lwdn_sfc_clr tmp.nc
            ncks -A -v lwdn_sfc_clr tmp.nc $file
          endif
          breaksw
        case lwup_sfc:
          set var_result = `ncdump -h -v rlus $file | grep rlus` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rlus $file tmp.nc
            ncrename -v rlus,lwup_sfc tmp.nc
            ncks -A -v lwup_sfc tmp.nc $file
          endif
          breaksw
        case lwup_sfc_clr:
          #
          ...
          if ( $var_status == 0 ) then
            ...
            ncks -A -v lwup_sfc_clr tmp.nc $file
          endif
          breaksw
        case swdn_sfc:
          set var_result = `ncdump -h -v rsds $file | grep rsds` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rsds $file tmp.nc
            ncrename -v rsds,swdn_sfc tmp.nc
            ncks -A -v swdn_sfc tmp.nc $file
          endif
          breaksw
        case swdn_sfc_clr:
          set var_result = `ncdump -h -v rsdscs $file | grep rsdscs` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rsdscs $file tmp.nc
            ncrename -v rsdscs,swdn_sfc_clr tmp.nc
            ncks -A -v swdn_sfc_clr tmp.nc $file
          endif
          breaksw
        case swup_sfc:
          set var_result = `ncdump -h -v rsus $file | grep rsus` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rsus $file tmp.nc
            ncrename -v rsus,swup_sfc tmp.nc
            ncks -A -v swup_sfc tmp.nc $file
          endif
          breaksw
        case swup_sfc_clr:
          set var_result = `ncdump -h -v rsuscs $file | grep rsuscs` ; set var_status = $status
          if ( $var_status == 0 ) then
            ncks -O -v rsuscs $file tmp.nc
            ncrename -v rsuscs,swup_sfc_clr tmp.nc
            ncks -A -v swup_sfc_clr tmp.nc $file
          endif
          breaksw
      endsw
    end
...
foreach obs_source ( ERBE CERES_ES4 CERES_SRBAVG_nonGEO CERES_SRBAVG_GEO CERES_EBAF CERES_EBAF_Ed2.6 \
                     CERES_EBAF_TOA_Ed2.7 CERES_EBAF_SFC_Ed2.7 CERES_EBAF_ATM_Ed2.7 \
                     CERES_EBAF_TOA_Ed2.8 CERES_EBAF_SFC_Ed2.8 CERES_EBAF_ATM_Ed2.8 )

echo '- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -'
echo '                    obs: '$obs_source
echo '- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -'

#  obs data source directory
...
set obs_data_file = $FRE_ANALYSIS_ARCHIVE/cjs/radiation_atmos_av_mon/$obs_source_sdir/data.tar.gz
...
switch ( $obs_source )
  ...
  case CERES_ES4:
    ...
    set obs_title_dat = 'http://eosweb.larc.nasa.gov/PRODOCS/ceres/level3_es4_table.html'
    set obs_title_ref = 'Wielicki et al. (1996), BAMS'
    #
    #  diagnostic variables
    #
    set diag_varlist = ( lwcf swcf netcf olr swabs netrad )
  ...
  case CERES_SRBAVG_nonGEO:
    ...
    set obs_title_dat = 'http://eosweb.larc.nasa.gov/PRODOCS/ceres/level3_srbavg_table.html'
    set obs_title_ref = 'Wielicki et al. (1996), BAMS'
    #
    #  diagnostic variables
    #
  ...
  case CERES_EBAF:
    ...
    set obs_title_dat = 'http://eosweb.larc.nasa.gov/PRODOCS/ceres/level4_ebaf_table.html'
    set obs_title_ref = 'Loeb et al. (2009), J. Climate'
    #
    #  diagnostic variables
    #
  ...
  case CERES_EBAF_Ed2.6:
    ...
    set obs_title_dat = 'http://ceres.larc.nasa.gov/products.php?product=EBAF'
    set obs_title_ref = 'Loeb et al. (2009), J. Climate'
    #
    #  diagnostic variables
    #
  ...
  case CERES_EBAF_TOA_Ed2.7:
    ...
    echo
    set obs_file = CERES_EBAF-TOA_Ed2.7_Subset_200003-201304.nc
    tar -xzf data.tar.gz $obs_file || exit 1
    if ( ! -e $obs_file ) then
      echo '...'$obs_file' does not exist, exiting script'
      exit 1
    ...
    set obs_title_dat = 'http://ceres.larc.nasa.gov/products.php?product=EBAF-TOA'
    set obs_title_ref = 'Loeb et al. (2009), J. Climate'
    #
    #  diagnostic variables
    #
  ...
  case CERES_EBAF_SFC_Ed2.7:
    ...
    echo
    set obs_file = CERES_EBAF-Surface_Ed2.7_Subset_200003-201209.nc
    tar -xzf data.tar.gz $obs_file || exit 1
    if ( ! -e $obs_file ) then
      echo '...'$obs_file' does not exist, exiting script'
      exit 1
    ...
    set obs_title_dat = 'http://ceres.larc.nasa.gov/products.php?product=EBAF-Surface'
    set obs_title_ref = 'Kato et al. (2013), J. Climate'
    #
    #  diagnostic variables
    #
  ...
  case CERES_EBAF_ATM_Ed2.7:
    cd $obs_file_work_dir || exit 1
    set obs_file_toa = CERES_EBAF-TOA_Ed2.7_Subset_200003-201304.nc
    ...
    set obs_file_sfc = CERES_EBAF-Surface_Ed2.7_Subset_200003-201209.nc
    ...
    set obs_title_dat = 'http://ceres.larc.nasa.gov/products.php?product=EBAF-{TOA,Surface}'
    set obs_title_ref = 'TOA: Loeb et al. (2009), J. Climate, Surface: Kato et al. (2013), J. Climate'
    #
  ...
  case CERES_EBAF_TOA_Ed2.8:
    ...
    echo
    set obs_file = CERES_EBAF-TOA_Ed2.8_Subset_200003-201607.nc
    tar -xzf data.tar.gz $obs_file || exit 1
    if ( ! -e $obs_file ) then
      echo '...'$obs_file' does not exist, exiting script'
      exit 1
  ...
  case CERES_EBAF_SFC_Ed2.8:
    ...
    echo
    set obs_file = CERES_EBAF-Surface_Ed2.8_Subset_200003-201602.nc
    tar -xzf data.tar.gz $obs_file || exit 1
    if ( ! -e $obs_file ) then
      echo '...'$obs_file' does not exist, exiting script'
      exit 1
  ...
  case CERES_EBAF_ATM_Ed2.8:
    cd $obs_file_work_dir || exit 1
    set obs_file_toa = CERES_EBAF-TOA_Ed2.8_Subset_200003-201607.nc
    if ( ! -e $obs_file_toa ) then
      echo
      echo 'extract CERES EBAF TOA Edition2.8 obs monthly time series file from compressed tar file...'
      echo
    ...
    set obs_file_sfc = CERES_EBAF-Surface_Ed2.8_Subset_200003-201602.nc
    if ( ! -e $obs_file_sfc ) then
      echo
      echo 'extract CERES EBAF Surface Edition2.8 obs monthly time series file from compressed tar file...'
      echo
...
titleobs_dat = '$obs_title_dat'
titleobs_ref = "$obs_title_ref"
titlemod     = "$mod_title"
...
if ( $do_stats_file = true )
  ...
  rc = write(stats_file,'Obs: 'titleobs_dat)
  rc = write(stats_file,'Obs: 'titleobs_ref)
...
* annotation
 ...
 if( titleobs_dsc != '' )
   if( titleobs_dat != '' | titleobs_ref != '' )
     'draw string 4.25 0.41 'titleobs_dsc':'
     if( titleobs_dat != '' )
       'draw string 4.25 0.22 'titleobs_dat
       if( titleobs_ref != '' )
         'draw string 4.25 0.07 'titleobs_ref
       endif
     else
       'draw string 4.25 0.22 'titleobs_ref
     endif
   endif
 endif
